...
{
   mnist_dataset_handle raw_dataset = mnist_open(name);

   // open original dataset named name
   if (raw_dataset == MNIST_DATASET_INVALID)
   {
      printf("There was a problem opening the raw dataset: ");
      mnist_free(raw_dataset);
      return MNIST_DATASET_INVALID;
   }

   // check if sample size larger than original dataset 
   if (mnist_image_count(raw_dataset) < n)
   {
      printf("Sample size can not be larger than population!");
      mnist_free(raw_dataset);
      return MNIST_DATASET_INVALID;
   }

   // if n == original size then return original size
   else if (mnist_image_count(raw_dataset) == n)
      return raw_dataset;

   // create a new empty dataset
   unsigned int size_x, size_y;
   mnist_image_size(raw_dataset, &size_x, &size_y);
   mnist_dataset_handle new_dataset = mnist_create(size_x, size_y, n);

   // initialize new_dataset
   new_dataset -> next = NULL;
   new_dataset -> total_images = n;
   new_dataset -> label_magic_number = raw_dataset -> label_magic_number;
   new_dataset -> image_magic_number = raw_dataset -> image_magic_number;
   new_dataset -> total_rows = raw_dataset-> total_rows;
   new_dataset -> total_cols = raw_dataset-> total_cols;
   

   // copy label/image data from original to new randomly:
   // for every image that we randomly pick
   // get a random number and read from the beginning of 
   // raw dataset read_times times, and copy image_data and 
   // label data to the new dataset. 
   srand (time(NULL));

   int h;

   unsigned long i, read_times;
   //unsigned long j;
  
   for(i = 1; i <= n; i ++)
   {
      read_times = rand()% mnist_image_count(raw_dataset);
      //image_random is the random image.
 
      const unsigned char* data_in_image = &raw_dataset->imagedata[read_times* (unsigned long)raw_dataset->total_rows * (unsigned long)raw_dataset->total_cols];
      
      // copy the current random_image's image data to new dataset
      for(h=0;h <size_x*size_y;h++)
      {
         new_dataset->imagedata[(i-1)*(int)size_x*(int)size_y+h] = data_in_image[h];
      }

      // copy random_image's label data to new dataset
      new_dataset->labeldata[i-1] = raw_dataset->labeldata[read_times];
   }
   return new_dataset;
}
