...
namespace OME
{
    ...
    public class BizDomain
    {
        ...
        public string ValidateOrder(Order order)// we'll probably change the type just made it string for now
        {
            ...
            errorMessage += order.LimitPrice > 50000000 ? "Price too high/" : order.LimitPrice < .01 ? "price too low/" : ""; //prob use another variable here just need to decide what kind of range we should allow
        ...
        {
            //XDocument d = XDocument.Load(@"C:\Users\Mike\Documents\Visual Studio 2012\Projects\Exchange\clearingLog.xml");
            //string balance = d.XPathSelectElement(@"ClearingCorpLog/Trader[@ID='2']/Balance").Value;
            //d.XPathNavigator.SelectSingleNode(@"ClearingCorpLog/Trader[@ID='2']/Balance");
            ...
            string balance = navigator.SelectSingleNode(@"ClearingCorpLog/Trader[@ID='2']/Balance", manager).Value;

        

            Console.WriteLine(balance); //check that the trader has enough margin here 
        }
        ...
        {
            Console.WriteLine(orderBook.CurrentPrice(Instrument, BuySell).LimitPrice.ToString());
        }
