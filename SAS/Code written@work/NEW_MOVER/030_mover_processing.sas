...
data movers (drop=new_cac_nm_target_valuescore);
   ...
   if a and b and cac_nm_target_valuescore ~= new_cac_nm_target_valuescore then do;
...
proc freq data=movers;
   title "AFTER MATCH WITHOUT MIDDLE NAME";
   table update_flag;
run;
...
data _null_;
   ...
   put ;
   put "   Number of records with different value score before update: &diff_score_before";
   put ;
