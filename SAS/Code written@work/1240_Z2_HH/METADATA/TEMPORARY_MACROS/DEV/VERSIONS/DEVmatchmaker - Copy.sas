      ...
      *------------------------------------------------------*;
         ...
         ***___ LOOP THROUGH KEYS TO FIND BEST KEY MATCH POSSIBLE___***;
         %do nums=1 %to &keynum;
            ...
            data cl_match_%scan(&scf_list, &aaa)_key&key %if &match_lev=2 %then %do; (drop=ind_match) %end;
         ...
         ods html body="./cacdirect_results_&client_data._&curstate..html";
            ...
            proc freq data=client_with_demo_&curstate;
               ...
               format cac_active_flag cac_act. cac_production cac_prod.
                      ...
                      %if &census=1 %then %do; cac_census_match %end; %if &ethnic=1 %then %do; cac_ethnic_match %end; match.
                      cac_silh silh.
...
data _null_;
   ...
   run_time=(end_dt-"&start_dt"dt);
   call symput('run_time',run_time);
run;
...
data _null_;
   ...
   %end;
   put ;
   put "The job was initiated on &start_date at &start_time and completed on &end_date at &end_time - taking &run_time to complete";   put ;
   put ;
   put "Good Work";
   put ;
