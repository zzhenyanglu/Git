%macro matchmaker(client_dir=, client_data=, client_id=, name_format=, name_var=, name_var2=, addr_format=, addr1=, addr2=, 
                  ...
                  geo_interest=, census=, census_keep=cen_fam cen_hh cen_home cen_pop, individual=, addr_detail=0, cacd_prod=1, results_by_state=1, email=jschiltz@cac-group.com);
*==============================================================================================*;
*  Macro:  	matchmaker
   ...
   Sections:	1) MATCHING PREP
                ...
                6) GO GRAB PID AND PUT ON CLIENT FILE
                7) ADD BASE DEMO FIELDS (NOT SELECTABLE)
                8) ADD GEO INTERESTS (IF SELECTED)  
                9) ADD CENSUS DATA (IF SELECTED)  
               10) ADD ETHNIC DATA (IF SELECTED)  
               11) ADD INDIVIDUAL DATA (IF SELECTED)  
...
filename zipstate temp;
data _null_;
   set sashelp.zipcode end=eof;
   format putvar $100.;
   file zipstate;
   if _n_=1 then put 'proc format; value $zs';
   putvar=compress("'" || put(zip,z5.) || "'='" || statecode || "'");
   put putvar;
   if eof then put "other='XX'; run;";
run;
filename ugh temp;
proc printto log=ugh; run;
%include zipstate;
...
%if &namelen=1 %then %goto toolong;
    ...
    *******************************************************************;
proc datasets library=&clib nolist;
   delete &client_data._lookup;
quit;
...
data clean client_not_clean;
   set &clib..&client_data (keep=&client_id &zip5 &zip4 &name_var &name_var2 &addr1 &addr2 &state_var &zip5) end=eof;
   ***___CREATES SCF AND ST_SCF__***;
   length mk_zip $5.;
   mk_zip=put(input(&zip5,best5.),z5.);
   mk_scf=substr(mk_zip,1,3);
   retain changed_state 0;
   changed_state+ (put(mk_zip,$zs.)^='XX' AND put(mk_zip,$zs.)^='' AND put(mk_zip,$zs.)^=&state_var);
   if eof then call symput('chg_st',changed_state);
   if put(mk_zip,$zs.)^='' and put(mk_zip,$zs.)^='XX' and put(mk_zip,$zs.)^=&state_var then do;
      mstate=put(mk_zip,$zs.);
   ...
   ***___OUTPUT DIRTY DATA:  IF MISSING NAME OR OTHER MATCHING ELEMENT___***;
   if clean_addr='' or mstate not in (&state_list) or put(st_scf,$val_st_scf.)='I' then output client_not_clean;
   else output clean;
run;
...
%let ratio=%eval(100*&dirty/&full);

%if &ratio>5 %then %do;
   filename sendmail email to=("&email")
                     subject="CACdirect Matching Results - Potential Warning" 
                     from="webmaster@cac-group.com" 
                     ;

   data _null_;
      file sendmail;
      put "Don't worry, you CACdirect matching job is still processing";
      put ;
      put "You have a large number of dirty records (&ratio% of your input file to be exact).";
      put ;
      put "These records are NOT candidates for matching - they are removed prior to processing.";
      put ;
      put "Don't worry...they will be added back to your data before your matched file is returned.";
      put ;
      put "You may want to consider killing your job, cleaning your data, and restarting matching.";
      put ;
      put "Good Work!";
      put ;
      put "Sincerely,";
...
*-----------------------------------------*;
      ...
      *------------------------------------------------------*;
         ...
         ***___SET MATCHES TOGETHER BY KEY SO THAT WE HAVE ALL IN ONE SCF DATASET___***;
         data scf_matched_%scan(&scf_list, &aaa) (keep=&client_id &zip5 &zip4 cac_hh_pid cac_production cac_active_flag 
                                                       mstate mk_zip matched matched_key hold_first_name );
            set %do g =1 %to &keynum;
                   %let key=%scan(&keylist,&g);
                   cl_match_%scan(&scf_list, &aaa)_key&key 
                %end;
                   nomatch_%scan(&scf_list, &aaa) (in=x%scan(&scf_list, &aaa))
            ;
            if x%scan(&scf_list, &aaa) then do;
                 matched=0;
                 matched_key=.;
                 cac_hh_pid=.;
                 cac_active_flag=.;
         ...
         proc datasets library=work nolist;
            delete %do g =1 %to &keynum;
                      %let key=%scan(&keylist,&g);
                      cl_match_%scan(&scf_list, &aaa)_key&key 
                   %end;
                      nomatch_%scan(&scf_list, &aaa) 
            ;
         quit;

      %end;                                     *------------------------------- END OF SCF LOOP;

      ***___STACK SCF FILES TOGETHER ___***;
      data client_matched_&curstate (keep=&client_id &zip5 &zip4 cac_hh_pid cac_production cac_active_flag mstate 
                                          mk_zip matched matched_key hold_first_name );
         set %do g =1 %to &count;
                scf_matched_%scan(&scf_list, &g)
      ...
      proc datasets library=work nolist;
         delete %do t= 1 %to &count;
                   scf_matched_%scan(&scf_list, &t)  
                %end;
         ;
      quit;
      ...
      *---------------------------------------------------------------*;
      %macro wps; 
         proc append base=&clib..&client_data._lookup data=client_matched_&curstate (keep=mstate &client_id cac_hh_pid cac_active_flag cac_production matched_key) force;
         run;
      %mend wps;
      data &clib..&client_data._lookup;
         set &clib..&client_data._lookup
...
*-----------------------------------------*;
      %if &census=1 %then %do ;
         ...
         data _null_;
            format var $100.;
            var="&census_keep";
            cen_fam=index(var,'cen_fam');
            call symput('cen_fam',cen_fam);
            cen_hh=index(var,'cen_hh');
            call symput('cen_hh',cen_hh);
            cen_home=index(var,'cen_home');
            call symput('cen_home',cen_home);
            cen_pop=index(var,'cen_pop');
            call symput('cen_pop',cen_pop);
         run;
 

         data client_with_demo_&curstate ;
            merge client_with_demo_&curstate(in=a)
                  cen2010.cac_census_final_&curstate (in=b rename=(cac_addr_zip=match_zip5 cac_addr_zip4=match_zip4)
                                                           keep=cen_count: cen_factor: cen_restype: cac_addr_zip cac_addr_zip4
                                                                %if &cen_fam=1  %then %do; cen_faminc: cen_famrace: %end;
                                                                %if &cen_hh=1   %then %do; cen_hhage: cen_hhinc: cen_hhincage: cen_hhincrace: cen_hhlang: cen_hhrel: cen_hhsize: %end;
                                                                %if &cen_home=1 %then %do; cen_hse: cen_hsecost: cen_hseheat: cen_hserace: cen_hserent: cen_hsesize: cen_hsetenr:
                                                                                           cen_hsevac: cen_hseval: cen_hseveh: %end;
                                                                %if &cen_pop=1  %then %do; cen_popage: cen_poped: cen_popgen: cen_popinc: cen_poplabr: cen_poplang: cen_popmar:
                                                                                           cen_popmob: cen_popocc: cen_popoth: cen_poppov: cen_poprace: %end;
                                                     );
            by match_zip5 match_zip4;
            if a;
            cac_census_match=b;
...
data &clib..&client_data._matched _dump (keep=cacdirect_hh_match where=(cacdirect_hh_match=1));
...
proc contents data=&clib..&client_data._matched;
...
data print;
            ...
            summary = '';   
            output;
            summary = "   Records Sent for Matching:   %eval(&full - &dirty ) / %eval(100*%eval(&full - &dirty )/&full)%";
            output;
...
proc freq data=&clib..&client_data._matched;
...
%if &namelen=1 %then %do;
   filename sendmail email to=("&email")
                     subject="CACdirect Matching Results - RTFM" 
                     from="webmaster@cac-group.com" 
                     ;

   data _null_;
      file sendmail;
      put "Your CACdirect matching job has completed - in a miserable failure.";
      put ;
      put "The dataset (&client_data) you submitted could not be processed since the name was too long (>24 bytes).";
      put ;
