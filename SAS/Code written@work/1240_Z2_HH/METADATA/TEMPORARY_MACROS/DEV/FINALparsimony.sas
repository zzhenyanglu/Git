...
data &outlib..&outdata
    (drop= name_up name_c namect psnm addr_length stupid pas1 lenpas name1 name2 name3 name4 name5 new_SUB_name mk_scf paddr pname st_scf
           ...
           new_addr_street new_addr_num new_addr_po new_addr_street_suffix new_addr_unit_number new_addr_unit_type new_addr_pre_directional new_addr_street_post_direction new_addr_road_number new_addr_fraction
...
*-----------------------------------------*;
   * GET RID OF SPECIAL CHARACTERS EXCEPT FOR FRACTIONS LIKE 1/2 *;
   new_addr=compbl(translate(upcase(strip(paddr))," ",".'*@^~,!$%()&-+="));
...
*---------------------------------------*;
   ...
   * ADDRESS ONLY MATCH KEYS *;
   mkey5=  compress(new_addr_num || new_addr_pre_directional || street_exact|| new_addr_unit_number ||&pzip);
   mkey6=  compress(new_addr_num ||                             street_exact|| new_addr_unit_number ||&pzip);
