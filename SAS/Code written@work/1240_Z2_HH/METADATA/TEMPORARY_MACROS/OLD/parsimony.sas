...
%macro parsimony(inlib=, indata=, outlib=, outdata=);

options mprint mlogic ps=9999;
...
*********************************************;
* IF LAST NAME HAS A COMMA AND A SUFFIX THEN
* CHANGE IT SO THERE IS NO COMMA
*********************************************;
...
***************************;
* IF SUFFIX IS IN THE LAST
* WORD THEN DEFINE SUFFIX
***************************;
...
******************************;
* CREATE TITLE FIRST_NAME 
* MI NAME LAST_NAME SUFFIX 
* BASED ON NUMBER OF WORDS IN 
* FULL NAME FIELD
******************************;
...
********************************************;
* IF THE LAST WORD STARTS WITH A # SIGN
* THEN IT CREATES A UNIT NUMBER. THEN LOOKS
* FOR STREET TYPE AND STREET NAME GIVEN
* PREVIOUS CONDITION
********************************************;
...
**************************************************;
* LOOKS TO SEE IF THE SECOND TO LAST WORD IS A
* TYPE OF UNIT, CALLS THE LAST WORD A UNIT NUMBER
* IF THIS IS THE CASE, LOOKS TO SEE IF THE THIRD
* TO LAST WORD IS A ROAD TYPE, IF SO CALLS THAT
* WORD THE SUFFIX AND THE PREVIOUS THE STREET NAME
* BUT IF NOT THEN IT CALLS THE THIRD TO LAST WORD
* THE STREET NAME
***************************************************;
...
***************************************;
* LOOKS TO SEE IF THE LAST WORD IS
* A TYPE OF UNIT, SEES IF THE PREV
* WORD IS A UNIT NUMBER, CHECKS PREV
* WORDS FOR ROAD TYPE AND STREET NAMES
***************************************;
...
*************************************;
* LOOKS TO SEE IF THE SECOND WORD IS
* DIRECTIONAL, NAMES WORD AFTER THE
...
**************************************;
* LOOKS TO SEE IF LAST WORD IS A ROAD
* TYPE THEN NAMES THE STREET SUFFIX
* AND STREET
**************************************;
...
************************************;
* IF LAST WORD BEGINS WITH A NUMBER
* AND UNIT NUMBER NOT DEFINED THEN
* CALLS IT A ROAD NUMBER, LOOKS FOR
* STREET SUFFIX AND STREET NAME
************************************;
...
***************************************;
* IF STREET AND SUFFIX BLANK THEN LOOKS
* FOR DIRECTIONAL IN LAST WORD, DEFINES
* POST DIRECTIONAL AND LOOKS FOR STREET
* AND STREET SUFFIX
***************************************;
...
*********************************;
* IF THERE ARE ONLY TWO WORDS AND
* THE ADDRESS NUMBER IS DEFINED
* THEN CREATES THE SECOND WORD
* TO BE THE STREET
**********************************;
...
********************************;
* CHANGES ADDRESSES WITH NO
* NUMBER TO JUST BE THE ORIGINAL
* ADDRESS BUT WITHOUT SPECIAL
* CHARACTERS
********************************;
...
****************************;
* PUT STATE ROUTE AND OTHER
* SIMILAR INTO ONE STREET 
* ADDRESS FIELD
****************************;
...
***********************;
* GETS RID OF SUFFIX IF
* IT IS THE ROAD NUMBER
***********************;
...
*************************;
  ...
  * street_fuzzy2=compress(street_exact,'AEIOU');
  * street_fuzzy3=substr(street_exact,1,7);
  * street_fuzzy4=substr(street_exact,1,5);
  ...
  * last_name_fuzzy2=compress(last_name,'AEIOU!');
  * last_name_fuzzy3=substr(last_name,1,7);
  * last_name_fuzzy4=substr(last_name,1,5);

   *keep id master_id cacdir_full_name cacdir_full_addr mk_zip mk_scf st_scf CAC_HH_PID last_name_exact last_name_fuzzy1-last_name_fuzzy4
        street_fuzzy1-street_fuzzy4 street_exact new_addr_pre_directional 
        new_addr_num new_addr_fraction new_addr_street_suffix 
        new_addr_street_post_direction new_addr_unit_number production active_flag lst_qtr_uptd;
run;

proc print data=temp(obs=20);
 title 'temp_%scan(&sts, &current_state)';
run;

data &outlib..&outdata. (KEEP=CAC_PRODUCTION CAC_ACTIVE_FLAG CAC_QTR MK_SCF st_scf CAC_HH_PID MKEY1 MKEY2 MKEY3 MKEY4 MKEY5 MKEY6 MKEY7) ;
  *set process.mkey_prep_%scan(&sts, &current_state);
   set temp;
...
/* BOTH ARE EXACT */
MKEY2=  compress(last_name_exact || new_addr_num || street_exact
                       || new_addr_unit_number || mk_zip); 
...
PROC PRINT DATA=&outlib..&outdata. (OBS=20);
  title "tsp_MKEY_%scan(&sts, &current_state) ";
RUN;
