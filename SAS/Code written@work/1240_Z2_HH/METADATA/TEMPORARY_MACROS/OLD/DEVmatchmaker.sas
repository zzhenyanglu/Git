...
%parsimony(inlib=work, indata=clean, outlib=work, outdata=client_outdat, pname_form=1, pname1=clean_name, pname2=, paddr_form=1, paddr1=clean_addr, paddr2=, pstate=state, pzip=mk_zip);
...
*APPEND HOUSEHOLD DEMO AND INTEREST DATA *;
   data client_with_demo_&curstate;
      %if &abort=0 %then %do;
         set client_matched_&curstate (in=a);
      %end;
      %if &abort>0 %then %do;
...
%if &census=1 %then %do ;
   ...
   data client_with_demo_&curstate;
      merge client_with_demo_&curstate (in=_a_ drop=cac_census_2000_level)
            ...
            rename=(county=census_county state=census_state zip5=census_zip5));  ***rename fields with the same name in both datasets;
      by &client_id;
      if _a_;
      if not _b_ then do;
