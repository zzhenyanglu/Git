...
*-----------------------------------------*;
      %if &geo_interest=1 %then %do;
         ...
         data client_zip_found_&curstate (drop=cac_addr_zip2 newzip rename=(hold=cac_addr_zip2)) state_&curstate (drop=geo: cac_addr_zip2);
            format cac_addr_zip2 $7.;
...
*-----------------------------------------*;
      ...
      proc append base=base_matched data=base_&curstate force;
      run;
   
      proc datasets library=work nolist;
         delete base_&curstate;
      quit;
...
proc sort data=base_matched out=base;
   by &client_id;
run;
