...
%include '/project/CACDIRECT/CODE/PROD/NEW_MOVER/.mover_includes.inc';   
...
libname mysqllib mysql user=usr_srp password=*1354E8F28DE1701ECC3436959B87BA7A8B3988F2 database=sarpinos server=sarpinos.cxj89kwq46vz.us-west-2.rds.amazonaws.com port=3306;
...
%macro update_date_cut;
   ...
   proc sql;
     * connect to mysql  (user=usr_srp password=FNyCV89hHErLDB7K server=216.157.38.64 
      ...
      connect to mysql  (user=usr_srp password=*1354E8F28DE1701ECC3436959B87BA7A8B3988F2 server=sarpinos.cxj89kwq46vz.us-west-2.rds.amazonaws.com 
        database=sarpinos port=3306); 
...
%macro get_data;
   ...
   proc contents data=psqlib.cogensia_movers;run;

   proc sql;
      connect to postgresql (SERVER=&server PORT=5432 DATABASE=cac_prod user=newmover password=&pw); 
      create table work.cogensia_movers_sas as
      select * from connection to postgresql
      (select * from cogensia_movers);
      disconnect from postgresql;
   quit;

%mend get_data; %get_data;

%macro wtnm_cr_mailable(date=&date., maxdate_lastfile='01JUN2015'd);
*29JUN2015 is next;
   ...
   data sarp.carrier_routes;
      infile "/project/SARPINOS/CODE/INCLUDES/carrier_routes_20150331_translated_for_all_stores.csv" dsd delimiter=',' firstobs=2;
      informat
              STORE_NUMBER $8.
              STORE_NAME $100.
              ZIP $5.
   ...
   proc sort data = cogensia_movers_sas;
      by CAC_NM_ZIP_CODE CAC_NM_CARRIER_ROUTE;
   run;
   
   data sarp.WTNM_CR_&date.;
      merge cogensia_movers_sas (in=a keep=CAC_NM_CARRIER_ROUTE CAC_NM_CITY CAC_NM_CONTRACTED_ADDR CAC_NM_CONTRACTED_NAME CAC_NM_DWELLING_ADDR_TYPE CAC_NM_DPV_FLAG 
...
%macro send_csv(store=,date=);
   ...
   */
   ...
   x "echo %bquote('/project/SARPINOS/CODE/sftp_script.sh sarpsftp 52.25.95.26 s\@rP\!n0s put /project/SARPINOS/DATA/&date./sarpinos_mailable_store_&store._&date..csv
      sarpinos_mailable_store_&store._&date..csv') > sftp_dl_launcher.sh";

   x "echo %bquote('/project/SARPINOS/CODE/sftp_script.sh sarpsftp 52.25.95.26 s\@rP\!n0s get sarpinos_mailable_store_&store._&date..csv
      /project/SARPINOS/DATA/&date./return_sarpinos_mailable_store_&store._&date..csv') >> sftp_dl_launcher.sh";

   x "chmod 777 sftp_dl_launcher.sh";
