...
data clean client_not_clean;
   ...
   ***___CREATES SCF AND ST_SCF__***;
   ...
   changed_state+ ((put(mk_zip,$zs.)^=&state_var));
   if eof then call symput('chg_st',changed_state);
   if put(mk_zip,$zs.) = &state_var then do;
      mstate=&state_var;
      st_scf=compress(mstate||"_"||substr(mk_zip,1,3));
   end;
   else if put(mk_zip,$zs.) ne &state_var and put(mk_zip,$zs.) in (&full_states) then do;
   ...
   ***___OUTPUT DIRTY DATA:  IF MISSING NAME OR OTHER MATCHING ELEMENT___***;
   if clean_addr='' or mstate='' or put(mk_zip,$zs.) not in (&state_list) or
...
*-----------------------------------------*;
      %if &geo_interest=1 %then %do;
         ...
         title "summary of GEO-Matching Results: &curstate.";
